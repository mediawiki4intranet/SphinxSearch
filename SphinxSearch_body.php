        ...
        if ($res===false) {
            ...
            if (is_array($res["words"])) {
                foreach ($res["words"] as $word => $info) {
                    $wgOut->addWikiText(sprintf(wfMsg('sphinxSearchStats'),
                                    $word,
                                    $info['hits'],
                                    $info['docs'])
                    );
    ...
    function createNewSearchForm($SearchWord, $namespaces, $categories) {
        ...
        $wgOut->addHTML("<form action='$kiAction' method='GET'>
                ...
                if ($wgSphinxSearch_mode == SPH_MATCH_EXTENDED) {
                        $wgOut->addHTML("<div style='margin:0.5em 0 0.5em 0;'><input type='radio' name='match_all' value='0' ".($wgSphinxMatchAll ? "" : "checked='checked'")." />".wfMsg('sphinxMatchAny')." <input type='radio' name='match_all' value='1' ".($wgSphinxMatchAll ? "checked='checked'" : "")." />".wfMsg('sphinxMatchAll')."</div>");
                }
