var sphinxSpinner = new Image;
sphinxSpinner.src = wgSphinxSearchExtPath + '/spinner.gif';

function sphinxShowCats(input) {

    var parent_id = input.id.substring(0, input.id.indexOf('_'));
    var delta = (input.checked ? -1 : 1);
    var parent_fld = input.form['cat_parents[_' + parent_id + ']'];
    var seen_parents = [];
    ...
    while (parent_fld) {
      ...
      for (var i = 0; i < cnt; i++) {
        ...
        if (el.name.indexOf('cat_parents') == 0) {
          ...
          if (document.getElementById(grandparent_id + '_' + parent_id)) {
            parent_fld = input.form['cat_parents[_' + grandparent_id + ']'];
    ...
    div.innerHTML= '<img src="' + sphinxSpinner.src + '" alt="' + sphinxLoadingMsg + '" />';
